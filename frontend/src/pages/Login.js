...
function Login() {
    ...
    const handleLogin = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.post('https://epic-wear-backend.onrender.com/api/auth/login', { email, password });
        ...
        <div style={{ minHeight: '100vh', background: '#0f0f1a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ backgroundColor: '#1a1a2e', padding: '40px', borderRadius: '20px', width: '100%', maxWidth: '420px', border: '1px solid #2a2a3e' }}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                    ...
                    style={{
                        width: '100%', padding: '12px', marginBottom: '15px', marginTop: '5px',
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                    placeholder="••••••••"
                    style={{
                        width: '100%', padding: '12px', marginBottom: '20px', marginTop: '5px',
                        borderRadius: '10px', border: '1px solid #2a2a3e', backgroundColor: '#16213e',
                        color: 'white', fontSize: '14px'
                    }} />
